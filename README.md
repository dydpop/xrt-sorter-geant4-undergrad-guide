...
这是一个面向本科项目展示和组内学习的 **X 射线透射（XRT）矿物分选仿真系统**。项目用 Geant4 搭建 X 射线源、矿石样本和探测器，用 C++ 输出事件数据，再用 Python 做特征提取、基础分类和图表展示。

> 一句话：这个仓库展示了一条包含仿真建模、事件输出、Python 特征分析和论文材料整理的本科项目链路。
...
- **结果可解释**：分类不是黑箱，核心依据是透射率、能量沉积、直接命中和散射命中等物理相关特征。
- **本科级成果闭环**：有代码、有图、有结果表、有论文稿、有复现说明，适合答辩、组内学习和二次扩展。
- **边界清楚**：`0.98` 只表示当前仿真数据上的粗粒度吸收组分类结果，不等于现场设备效果。
...
探测器记录事件后，Python 统计透射、能量沉积、直接命中和散射命中，用这些特征支撑分类判断。

![直接与散射命中比例](figures/elementary_direct_scatter_ratio.png)

### 3. 基础分类已经形成可展示结果

在当前仿真数据和粗粒度吸收组任务中，基础分类方法最高 accuracy 为 `0.98`。样本构造、特征和划分方式见 `results/` 与 `analysis/classify_absorption_groups.py`；该数值不是实物设备测试指标。
...
更详细的依赖和排错见 `docs/RUN_LOCALLY_zh.md`。
...
本仓库公开的是本科级仿真项目成果。它可以说明：我们完成了 Geant4 XRT 仿真系统、数据输出、基础特征分析和仿真数据上的粗粒度分类验证。

它不应该被解释为：已经完成现场设备验证、可直接用于分选设备控制、或已经覆盖复杂矿石流的全部情况。
